...
export function usePagination(props) {
    ...
    const current = computed(() => Math.max(model.value.value, 1));
    const numberOfPages = computed(() => Math.ceil(props.total / props.perPage));
    const after = computed(() => Math.max((model.value.value - 1) * props.perPage, 0));
    const nextPage = computed(() => Math.min(numberOfPages.value, model.value.value + 1));
    const hasNext = computed(() => (props.perPage + after.value) < props.total);
    const previousPage = computed(() => Math.max(0, model.value.value - 1));
