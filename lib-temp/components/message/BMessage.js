...
function generateHeader(props, context, message) {
    ...
    if (props.isClosable) {
        nodes.push(h('button', Object.assign({ class: 'delete', attrs: { 'aria-label': `Close message` } }, message.listeners)));
    }
...
export default defineComponent({
    name: 'b-message',
    props: Object.assign(Object.assign({}, UseMessagePropsDefinition), FadeTransitionPropsDefinition),
    setup(props, context) {
        const message = useMessage(props);
        const transition = useTransition(props);
        return h(Transition, Object.assign({}, transition.value), message.isOn.value && generateMessage(props, context, message));
    }
});
