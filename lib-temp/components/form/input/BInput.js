...
export function getBInputPropsDefinition() {
    return Object.assign(Object.assign({}, getUseInputPropsDefinition()), { isLoading: {
            type: Boolean,
            default: false
        }, hasCounter: {
            type: Boolean,
...
function generateRightIcon(icon, size, variant, usePasswordReveal, passwordToggle) {
    return h(icon, Object.assign(Object.assign({ staticClass: 'is-right', class: ['is-right', { 'is-clickable': usePasswordReveal }], variant,
        size }, passwordToggle.attrs.value), passwordToggle.listeners));
}
...
function generateNonTextInput(inputRef, inputData, isLoading, rightIcon, context) {
    ...
    return h('input', Object.assign(Object.assign({}, context.attrs), { ref: inputRef, class: [
            'input',
        ...
        ], modelValue: inputData.modelValue, 'onUpdate:modelValue': inputData['onUpdate:modelValue'], type: inputData.type ? inputData.type.value : undefined, autocomplete: getAutocomplete(inputData.autocomplete, type), maxlength: inputData.maxlength && inputData.maxlength.value, placeholder: inputData.placeholder && inputData.placeholder.value, onBlur: inputData.onBlur, onFocus: inputData.onFocus, required: inputData.isRequired.value, readonly: inputData.isReadonly.value, disabled: inputData.isDisabled.value, tabindex: inputData.isDisabled.value ? 0 : -1, id: inputData.id.value }));
}
function generateTextarea(inputRef, inputData, isLoading, rightIcon, context) {
    const hasMessage = !!inputData.message.value;
    return h('textarea', Object.assign(Object.assign({}, context.attrs), { ref: inputRef, class: [
            'textarea',
            ...getInputClasses(inputData.iconSize.value, inputData.isExpanded.value, isLoading, hasMessage, inputData.icon && inputData.icon.value, rightIcon)
        ], modelValue: inputData.modelValue.value, 'onUpdate:modelValue': inputData['onUpdate:modelValue'], maxlength: inputData.maxlength && inputData.maxlength.value, placeholder: inputData.placeholder && inputData.placeholder.value, onBlur: inputData.onBlur, onFocus: inputData.onFocus, required: inputData.isRequired.value, readonly: inputData.isReadonly.value, disabled: inputData.isDisabled.value, tabindex: inputData.isDisabled.value ? 0 : -1, id: inputData.id.value }));
}
