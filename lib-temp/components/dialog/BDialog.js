...
export default defineComponent({
    ...
    setup(props, { attrs, slots }) {
        ...
        generateDialog.value = () => {
            ...
            const isDialogContent = isNonEmpty(nodes) &&
                pipe(head(nodes), node => fromNullable(node.component), mapNullable(component => component.type.name), exists(name => name === B_DIALOG_CONTENT_NAME));
            return [
                h(BDialogOverlay, {
                    isActive: true,
                    onClose: popup.close,
                    slots: {
                        default: () => isDialogContent
                            ? nodes
                            : h(BDialogContent, Object.assign(Object.assign({}, attrs), { slots: {
