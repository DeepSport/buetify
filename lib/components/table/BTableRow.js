...
import { h } from 'vue';
export default function BTableRow(props, {
  attrs,
  slots
}) {
  ...
  const columns = props.columns.map(column => {
    ...
    const textClass = column.position === 'is-left' ? 'has-text-left' : column.position === 'is-centered' ? 'has-text-centered' : 'has-text-right';
    return h('td', {
      class: [textClass, {
