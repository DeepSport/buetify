...
export const BTabsPropsDefinition = Object.assign(Object.assign(Object.assign({}, getUseModelPropsDefinition()), getUseThemePropsDefinition(TabsThemeMap)), {
  isExpanded: {
    type: Boolean,
    default: false
  },
...
function getGenerateNavItem(props, model, transition, vShow) {
  return function generateNavItem(step, index) {
    ...
    }, step.props.icon ? [h(step.props.icon, {
      size: props.size
    }), step.props.label] : step.props.label)]), [[vShow, step.props.isVisible]]);
  };
}
...
function generateNavHeaderContent(props, tabs, model, transition, vShow, themeClasses) {
  ...
  }, props.label ? [generateNavLabel(props), generateNavItems(props, tabs, model, transition, vShow)] : [generateNavItems(props, tabs, model, transition, vShow)]);
}

function generateNavHeader(props, tabs, model, transition, vShow, themeClasses) {
  return props.isScrollable ? h(BScroll, {
    class: 'is-fullwidth'
  }, [generateNavHeaderContent(props, tabs, model, transition, vShow, themeClasses)]) : generateNavHeaderContent(props, tabs, model, transition, vShow, themeClasses);
}
...
function generateTabContent(props, tabs, model, transition) {
  ...
  }, props.isAnimated ? [h(Transition, {
    name: transition.value
  }, tabs[model.modelValue.value || 0].render())] : tabs[model.modelValue.value || 0].render());
}
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    return () => {
      ...
      return h('article', {
        class: 'b-steps'
      }, [generateNavHeader(props, tabs, model, transition, vShow, themeClasses.value), HorizontalDivider, generateTabContent(props, tabs, model, transition)]);
    };
  }
