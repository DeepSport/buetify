...
export const BStepsPropsDefinition = Object.assign(Object.assign(Object.assign({}, getUseModelPropsDefinition()), DefaultThemePropsDefinition), {
  variant: {
    type: String,
    default: 'is-link'
  },
...
function generateNavItemContent(props, step, index, model, transition) {
  return h('a', {
    class: ['step-link', {
      'is-clickable': step.props.isClickable
    }],
...
function getGenerateNavItem(props, model, transition, vShow) {
  return function generateNavItem(step, index) {
    return withDirectives(h('li', {
      ...
      class: ['step-item', step.props.variant || props.variant, {
        'is-active': index === model.modelValue.value,
        'is-completed': step.props.isCompleted || model.modelValue.value > index
      }]
...
function generateNavHeader(props, steps, model, transition, vShow) {
  return h('nav', {
    class: ['steps', props.variant, props.size]
  }, [h('ul', {
    class: 'step-items'
  }, steps.map(getGenerateNavItem(props, model, transition, vShow)))]);
}
...
function generateStepContent(props, steps, model, transition) {
  ...
  }, steps[model.modelValue.value || 0].render())] : steps[model.modelValue.value || 0].render());
}
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    return () => {
      ...
      return h('article', {
        class: 'b-steps'
      }, [generateNavHeader(props, steps, model, transition, vShow), HorizontalDivider, generateStepContent(props, steps, model, transition)]);
    };
  }
