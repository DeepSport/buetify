...
function getBSelectPropsDefinition(eq) {
  return __assign(
    __assign(
      __assign(__assign({}, shared_1.getEqPropsDefinition(eq)), useInput_1.getUseInputPropsDefinition()),
      theme_1.DefaultThemePropsDefinition
    ),
    {
      items: {
        type: Array,
...
function generateSelect(props, context, ref, input, themeClasses) {
  ...
  return vue_1.h(
    'select',
    __assign(__assign({}, context.attrs), {
      ref: ref,
      value: value,
    ...
    usePlaceholder
      ? __spreadArrays([generatePlaceholder(props, context)], generateOptions(props, context, input))
